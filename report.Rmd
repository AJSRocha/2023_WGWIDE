...
df$peso_total_caixa %>% summary()
df$peso_total_spp %>% summary()
df$n_total_caixa %>% summary()
df$n_nao_observados_tot %>% summary()
...
## Lepidotriglas
df[df$cod_fao == c("LDV", "LEP"),] %>% View()

df[df$cod_fao == c("LDV", "LEP"),]$id_viagem %>% unique()

dados_rever <- df[df$cod_fao == c("LDV", "LEP"),]
# save(dados_rever, file = "data_problem.RData")

## GUR
df[df$cod_fao_venda == c("GUR"),]$cod_fao %>% unique()
df[df$cod_fao_venda == c("GUR"),]$id_viagem %>% unique() %>% length()
df[df$cod_fao_venda == c("GUR"),]$classe_comp %>%  summary()

df[df$cod_fao_venda == c("GUR"),] %>% View()

## GUR
df[df$cod_fao == c("GUR"),]$cod_fao_venda %>% unique()
df[df$cod_fao == c("GUR"),]$id_viagem %>% unique() %>% length()
df[df$cod_fao == c("GUR"),]$classe_comp %>%  summary()

df[df$cod_fao == c("GUR"),] %>% View()
...
df %>% filter(cod_fao=="GUR"& classe_comp > 40)

#Figueira da Foz
unique(df[df$lota=="FIGUEIRA DA FOZ","cod_fao_venda"])

#todas estas viagens foram validadas tirando o manata, descrito abaixo
##GUR
df %>% filter(cod_fao=="GUR"& lota == "FIGUEIRA DA FOZ") %>% View

##GUU
df %>% filter(cod_fao=="GUU"& lota == "FIGUEIRA DA FOZ") %>% View

##GUN
df %>% filter(cod_fao=="GUN"& lota == "FIGUEIRA DA FOZ") %>% View

#Matosinhos
##LDV - Todos resolvidos
df %>% filter(cod_fao=="LDV"& lota == "MATOSINHOS") %>% View

#LEP?
df %>% filter(cod_fao=="LEP") %>% View
```
...
df[df$cod_fao == c("GUU"),] %>% 
  ggplot() +
  geom_bar(aes(x=classe_comp, y=n_nao_observados), stat = "identity") + 
  facet_grid(mes~lota)
...
df$cod_fao_venda %>% unique()
#  [1] GUR LEP LDV HKE GUU     WRA ANN CTB BRB MUR
# Levels:  ANN BRB CTB GUR GUU HKE LDV LEP MUR WRA

i = "GUR"

df %>%
  filter(cod_fao_venda == i) %>%
  group_by(lota, classe_comp, cod_fao) %>%
  summarise(n = n_nao_observados) %>%
  ggplot() + 
  ggtitle(paste0("cod_fao_venda = ", i)) +
  geom_histogram(stat = 'count',
                 aes(x = classe_comp,
                     fill = cod_fao)) + 
  facet_wrap(.~lota) +
...
write.table(folha_amarela_exp,
            file=paste("outputs/IC",ano,"27_9a_PT_landings_length.dat",sep="_")
